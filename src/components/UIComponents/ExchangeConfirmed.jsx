...
export default function ExchangeConfirmed({ data, handlers }) {
  if (!data) return null
  ...
  return (
    ...
    >
      <div className="mb-4 flex items-center gap-3">
        ...
        <div>
          <h3 className="font-heading text-lg font-bold text-green-400">Exchange confirmed</h3>
          <p className="text-sm text-[var(--text-secondary)]">Your exchange has been processed successfully.</p>
        </div>
      </div>
...
function EticketBadge({ eticket, className = '' }) {
  return (
    <div className={`rounded-xl border border-green-500/20 bg-green-500/5 px-4 py-3 ${className}`}>
